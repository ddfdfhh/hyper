...
class PrivateChatPopup extends Component
{
    use WithFileUploads;
    ...
    public function render()
    {
        ...
        $this->dispatchBrowserEvent("scroll");
        return view("livewire.private_chat_popup");
    }
       ...
       public function MyLastChatInfo($with)
    {
         ...
         $this->chatting_to_user = User::find($this->chatting_to_id);
         $this->chatting_to_id =$this->chatting_to_id;
        $room1 = "room-" . $this->me . "-" . $this->chatting_to_id;
        $room2 = "room-" . $this->chatting_to_id . "-" . $this->me;
        $this->conversation_list = PrivateConversation::with("user")
            ->whereIn("room", [$room1, $room2])->where("status",1)
            ->get()
            ->toArray();

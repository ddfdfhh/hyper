...
<link href="https://cdn.jsdelivr.net/npm/summernote@0.8.18/dist/summernote.min.css" rel="stylesheet">
            ...
            @include('livewire.forum_modals.modal')
						...
						<table class="table table-hover">
							...
							<tbody>
                             ...
                             <tr>
                                 <td>{{$loop->index+1}}</td>
                                 <td>{{$r->title}}</td>
                                 <td>{!! \Str::limit(strip_tags($r->details),20,'...<br><a href="'.route('admin.forum.view',['id'=>$r->id]).'">Read More</a>') !!}</td>
                                 <td>
                                 @php
                               $src='';
                                if($r->featured_image && file_exists(storage_path('app/public/forum/'.$r->featured_image))) 
                                 $src=asset('storage/forum/'.$r->featured_image);
                             ...
                             @if($src)
                              ...
                              @endif
                               ...
                               <td>
                               <a href="{{route('admin.forum.view',['id'=>$r->id])}}" type="button" class="btn btn-info text-white btn-xs"><i class="fa fa-eye"></i></a>
                               <a href="{{route('admin.forum.edit',['id'=>$r->id])}}" type="button" class="btn btn-info text-white btn-xs"><i class="fa fa-edit"></i></a>
